...
> разработчике. На macOS 15 и новее обход через правый клик убран: нажмите «Готово», затем
> откройте «Системные настройки» → «Конфиденциальность и безопасность» → раздел
> «Безопасность» → «Открыть всё равно» (кнопка живёт около часа после попытки запуска).
> На macOS 14 и старше достаточно правого клика по приложению → «Открыть».
