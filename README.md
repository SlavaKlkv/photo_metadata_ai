...
```mermaid
flowchart TB
  E["<b>desktop/</b> · Electron<br/>\nокно приложения"] --> B["<b>backend/</b> · FastAPI<br/>\n127.0.0.1:8000"]
...
> Открыться он может не сразу — системе нужно время на проверку, и надёжнее
> открывать его из Finder, а не из панели загрузок браузера. Порядок первого запуска:
>
> 1. Открыть `.dmg` и перетащить приложение в `Applications` — **до** первого запуска.
>    Разрешение Gatekeeper выдаётся конкретному экземпляру: если запустить и разрешить
>    приложение прямо из папки загрузок, после переноса оно снова заблокируется.
> 2. Запустить приложение из `Applications`, в предупреждении нажать «Готово».
> 3. Системные настройки → «Конфиденциальность и безопасность» → раздел «Безопасность»
>    → **«Открыть всё равно»**. Кнопка доступна около часа после попытки запуска.
>
> Если этот путь не срабатывает — снять карантин в терминале:
>
> ```bash
> xattr -dr com.apple.quarantine "/Applications/Photo Metadata AI.app"
> ```

Порядок публикации версии — в [desktop/README.md](desktop/README.md).
