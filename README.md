...
> нотаризации не будет. На macOS 15 и новее скачанный `.dmg` под карантином не
> открывается ни двойным кликом, ни через правый клик, ни кнопкой «Открыть всё равно»
> в настройках безопасности — с `.zip` то же самое. Единственный рабочий способ —
...
> ```
>
> После установки то же самое может понадобиться самому приложению:
>
> ```bash
> xattr -dr com.apple.quarantine "/Applications/Photo Metadata AI.app"
> ```
>
> Первый запуск занимает заметно больше времени, чем последующие: бэкенд собран
> PyInstaller в onefile и распаковывается во временный каталог, а macOS дополнительно
> сканирует новый бандл.
