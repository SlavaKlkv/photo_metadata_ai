...
| **Обновления** | Приложение проверяет опубликованные GitHub Releases и сообщает о новой версии баннером. Автоустановки нет: `.dmg` скачивается в «Загрузки», приложение закрывается после скачивания, образ открывается — его заменяют в `Applications` |
