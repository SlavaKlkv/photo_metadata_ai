...
describe('поиск dmgbuild в кэше', () => {
  ...
  // Логгер dmg-builder подмешивает в stdout строку прогресса загрузки:
  // в терминале она уходит в tty, а в CI попадала в переменную, и «путь»
  // из двух строк указывал в никуда.
  it('берёт из вывода только путь, игнорируя строку прогресса', () => {
    const fn = buildScript.match(/ensure_dmgbuild\(\) \{[\s\S]*?\n\}/)[0];

    expect(fn).toMatch(/node scripts\/fetch-dmgbuild\.js \| tail -1/);
  });
