...
test('строка под кнопкой называет объём загрузки и место на диске', () => {
  const note = readLanding().match(/<p class="cta-note">([^<]*)</);

  expect(note).not.toBeNull();
  expect(note[1]).toMatch(/^Загрузка \d+ МБ · на диске \d+ МБ ·/);
...
test('подстановка обновляет оба размера, не трогая остальной текст', () => {
  ...
  expect(updated).toContain(
    '<p class="cta-note">Загрузка 140 МБ · на диске 330 МБ · без регистрации и подписки',
  );
});
