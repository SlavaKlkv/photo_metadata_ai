...
> ```
>
> Отдельная история — сам `.dmg`: под карантином он не монтируется и диалога не
> показывает (`open` возвращает `-128`), поэтому кнопки «Открыть всё равно» для образа
> не появляется. Тогда карантин снимается и с него:
>
> ```bash
...
> считает файл повреждённым. Полностью диалоги снимает только нотаризация — по оценке
> `syspolicy_check` ad-hoc подпись это `Warning`, а отсутствие нотаризации `Fatal`.
