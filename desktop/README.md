...
> [!IMPORTANT]
> На macOS 15 и новее ни один GUI-обход не работает. Проверено на скачанном
> из релиза образе: двойной клик из панели загрузок не даёт ничего, правый клик →
> **Открыть** этот сценарий больше не покрывает (Apple убрала обход), а
> **Открыть всё равно** в настройках безопасности до `.dmg` не доходит. С `.zip`
> ровно то же самое — карантин наследуется распакованному бандлу.
>
> Единственный рабочий способ — снять карантин в терминале:
>
> ```bash
...
> ```
>
> Ad-hoc подпись образа в `build-mac.sh` при этом остаётся нужной: без неё macOS
> считает файл повреждённым. Полностью диалоги снимает только нотаризация, для
> которой нужны учётные данные Apple Developer.

> [!NOTE]
> Первый запуск заметно дольше последующих: бэкенд собран PyInstaller в onefile и
> распаковывается во временный каталог, а macOS вдобавок сканирует новый бандл.
