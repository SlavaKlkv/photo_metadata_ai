...
> ```
>
> Сам `.dmg` намеренно не подписывается: ad-hoc подпись заставляла Gatekeeper
> проверять образ и блокировать монтирование — двойной клик по скачанному файлу
> завершался ошибкой `-128` без всякого диалога. Неподписанный образ проверять
> нечего, и он открывается штатно. Подпись бандла приложения при этом обязательна:
> без неё macOS считает его повреждённым.
>
> Полностью диалоги снимает только нотаризация — по оценке `syspolicy_check` ad-hoc
> подпись это `Warning`, а отсутствие нотаризации `Fatal`.
