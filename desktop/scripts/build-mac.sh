...
build_app() {
  ...
  # Бэкенд собран в режиме onedir: рядом с исполняемым файлом обязан лежать
  # _internal с Python и библиотеками. Без него приложение не работает, а
  # сборка выглядит успешной — ровно так в черновик релиза уехал бандл с
  # одним 12-мегабайтным файлом вместо бэкенда.
  if [[ ! -d "$DESKTOP_DIR/resources/backend/arm64/_internal" ]]; then
    echo "ОШИБКА: рядом с бинарником нет _internal — приехал не весь onedir-бандл." >&2
    exit 1
  fi
