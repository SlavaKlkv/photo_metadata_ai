...
def render_metadata_generation_prompt(
    *,
    ...
    prompt = (
        ...
        'license_type, location, location_metadata, editorial_date, '
        'is_editorial, '
        ...
        'location must be an object describing the single real geographic '
        'location of the depicted scene with the keys sublocation, city, '
        'province_state and country; '
        'set each key to its correct value only when it is genuinely known '
        'for this specific image, otherwise null; '
        'do not guess, do not reuse locations from other images, and do not '
        'reorder unrelated place names across the fields; '
        'city must be a city, province_state a state or region and country a '
        'country; when only a broad location is known, fill only the matching '
        'field and leave the others null; '
        'location_metadata must be a concise human-readable location string '
        'consistent with the location object when a real location is visible '
        'or provided, otherwise null; '
