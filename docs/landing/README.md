...
| Искры | [`brand-sparkles.js`](./brand-sparkles.js) | На десктопе intro выключено, рабочий режим `'twinkle-pairs'`: раз в 3–6 секунд в свободном месте поблёскивает крупный знак 11px; половина импульсов — двойной знак со спутником и кольцом. Режим `'flow'` с компактными триадами сохранён, но выключен. На touch/coarse остаётся отдельная одиночная вспышка 8 или 11px без пар. Слой не перехватывает клики. |
...
| 1080 px | index | Вместо ленты ссылок — бургер и та же `#nav-links` панелью. Бургер у правого края, пока в шапке нет «Скачать»; когда кнопка есть — слева от неё |
...
| `BRAND_SPARKLES_INTRO` | Intro-режим искр. Сейчас `false`; `'flow'` включает стартовый фронт появления. |
| `BRAND_SPARKLES_WORKING` | Рабочий режим искр. Сейчас `'twinkle-pairs'`: редкие одиночные/парные вспышки с кольцом. `'flow'` включает непрерывный поток со сменой курса; `false` — без цикла. |
